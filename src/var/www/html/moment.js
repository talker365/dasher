...
class moment {
    ...
    createUTC(input, format, locale, strict) {
    ...
    handleStrictParse(monthName, format, strict) {
        var i,
            ii,
            mom,
    ...
    dayOfYearFromWeeks(year, week, weekday, dow, doy) {
    ...
    handleStrictParse$1(weekdayName, format, strict) {
        var i,
            ii,
            mom,
    ...
    localeWeekdaysParse(weekdayName, format, strict) {
        var i, mom, regex;
    ...
    localeMeridiem(hours, minutes, isLower) {
        if (hours > 11) {
    ...
    checkWeekday(weekdayStr, parsedInput, config) {
        if (weekdayStr) {
    ...
    meridiemFixWrap(locale, hour, meridiem) {
        var isPm;
    ...
    createLocal(input, format, locale, strict) {
    ...
    offsetFromString(matcher, string) {
    ...
    addSubtract(mom, duration, isAdding, updateOffset) {
    ...
    getCalendarFormat(myMoment, now) {
    ...
    localeErasParse(eraName, format, strict) {
        var i,
            l,
            eras = this.eras(),
            name,
    ...
    matchEraAbbr(isStrict, locale) {

        return locale.erasAbbrRegex(isStrict);
    }

    matchEraName(isStrict, locale) {

        return locale.erasNameRegex(isStrict);
    }

    matchEraNarrow(isStrict, locale) {

        return locale.erasNarrowRegex(isStrict);
    }

    matchEraYearOrdinal(isStrict, locale) {
    ...
    getSetWeekYearHelper(input, week, weekday, dow, doy) {
        var weeksTarget;
        if (input == null) {
    ...
    // (true, fmt, 5)
    // (true, fmt)
    listWeekdaysImpl(localeSorted, format, index, field) {
    ...
    addSubtract$1(duration, input, value, direction) {
    ...
    relativeTime$1(posNegDuration, withoutSuffix, thresholds, locale) {
    ...
    getSetRelativeTimeThreshold(threshold, limit) {
    ...
    humanize(argWithSuffix, argThresholds) {
        if (!this.isValid()) {
            return this.localeData().invalidDate();
        }
